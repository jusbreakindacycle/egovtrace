...
import { canonicalizeGovernmentEvent, type GovernmentEventCandidate } from '../../domain/src/government-event-lifecycle.js';
...
async function ensureLifecycleSchema(store: PersistenceStore): Promise<void> {
...
export async function ingestGovernmentEvent(store: PersistenceStore, candidate: GovernmentEventCandidate, provenance?: ProvenanceInput): Promise<IngestGovernmentEventResult> {
  ...
  await ensureLifecycleSchema(store);
  ...
  try {
    ...
    if (existing.rowCount) {
      ...
      return { eventId: existing.rows[0].id, entityId: existing.rows[0].entity_id, sourceObservationKey: sourceKey, replayed: true, eventVersion: existing.rows[0].event_version, outboxId: existing.rows[0].outbox_id };
    }
    ...
    const event = await client.query<{ id: string }>(
      ...
      [entityId, candidate.eventType, candidate.occurredAt ?? null, candidate.status ?? 'RECORDED', candidate.assertionKind, candidate.confidence, candidate.objectEntityId, provenanceId, candidate.observationAt, candidate.sourceRecordedAt ?? null, candidate.temporalPrecision ?? (candidate.occurredAt ? 'DATETIME' : 'UNKNOWN'), candidate.source.observationOutcome ?? 'OBSERVED', 1, sourceKey]
    );
    const eventId = event.rows[0].id;
    const outbox = await client.query<{ id: string }>('INSERT INTO outbox_record (event_type,aggregate_type,aggregate_id,payload) VALUES ($1,$2,$3,$4) RETURNING id', ['event.created', 'GOVERNMENT_EVENT', entityId, { eventId, entityId, sourceObservationKey: sourceKey }]);
    await client.query('COMMIT');
    return { eventId, entityId, sourceObservationKey: sourceKey, replayed: false, eventVersion: 1, outboxId: outbox.rows[0].id };
  } catch (error) {
