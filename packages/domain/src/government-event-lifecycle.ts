...
export function validateGovernmentEventCandidate(candidate: GovernmentEventCandidate): void {
  ...
  if (!candidate.assertionKind || !['FACT', 'OBSERVATION', 'CLAIM'].includes(candidate.assertionKind)) {
    throw new Error('GovernmentEvent assertionKind must be FACT, OBSERVATION, or CLAIM');
  }
  if (candidate.eventType in paymentStateByEventType) {
    if (candidate.financialState !== paymentStateByEventType[candidate.eventType]) {
  ...
  if (candidate.assertionKind === 'CLAIM' && candidate.status === 'CONTESTED') {
    return;
  }
  ...
  if (candidate.occurredAt && candidate.sourceRecordedAt && candidate.sourceRecordedAt < candidate.occurredAt) {
    throw new Error('sourceRecordedAt cannot precede occurredAt without an explicit source conflict representation');
  }
}
...
export function canonicalizeGovernmentEvent(candidate: GovernmentEventCandidate): Record<string, unknown> {
  ...
  const observationOutcome = candidate.source.observationOutcome ?? 'OBSERVED';
  if (observationOutcome !== 'OBSERVED') throw new Error(`cannot create GovernmentEvent from observation outcome ${observationOutcome}`);
  return {
